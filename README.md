...
| 資料列結尾的空欄被砍掉，比標題列短 | 補齊，標題列不會被誤判成資料 |
| ALV 的 `\|----\|` 框線與分頁重印的標題 | 丟掉 |
...
匯出那一刻就沒了，檔案裡根本沒有那幾個零。儲存格是**文字**的話零就
還在，程式會在讀檔時就把代碼欄指定成 `dtype=str` 保住它（見下節）。
這裡能保證的是**型別每次都一致**。目前 576 個欄名，逐一比對過這 32 張表：
...
`pandas.read_csv` 與 `read_excel` 會自己推斷型別，`0010001234` 一律讀成
`int64`，零當場就沒了——之後再怎麼轉回字串都只剩 `10001234`。零明明好好地
寫在檔案裡（CSV 是純文字，Excel 那格也真的是字串儲存格），是讀檔時丟掉的。

所以 `read_sheet()` 分兩步：

1. **先只讀標題列**拿欄名，欄名認得出來是代碼的（`BELNR` `EBELN` `MATNR`…）
   就在正式讀檔時指定 `dtype=str`。只讀標題很便宜，5 萬列的 xlsx 約 0.2 秒，
   整份要 0.8 秒。
2. 欄名認不出來、值又是整數的（自訂的 `ZZ` 欄位那種），照樣會被當成代碼，
   但零已經被吃掉了，**只能把那幾欄重讀一次**。

第 2 步不便宜：5 萬列的 xlsx 從 1.4 秒變 2.6 秒。單號少一個零是安靜的錯，
比慢更糟，所以預設開著。真的不需要就把 `config.json` 的 `int_codes` 設成
`false`，第 2 步整個跳過（欄名認得出來的代碼欄不受影響，照樣保零）。
...
**轉換前用完整資料校正一次。** 掃描為了快只看前 200 列
（Excel 是真的只讀 200 列；txt 是整檔解析後取前 200 列，所以 txt
比較不會踩到）。第 201 列之後才出現的值可能放不進抽樣判出來的型別：

| 抽樣判的 | 完整資料裡有 | 校正成 | 不校正會怎樣 |
|---|---|---|---|
| `NVARCHAR(40)` | 60 字的值 | `NVARCHAR(255)` | `String data, right truncation` |
| `INT` | `12.5` | `DECIMAL(18,1)` | `cannot safely cast non-equivalent float64 to int64` |
| `INT` | `3000000000` | `BIGINT` | 溢位 |
| `DECIMAL(18,2)` | `12.345` | `DECIMAL(18,3)` | **不報錯**，SQL Server 安靜地四捨五入掉 |

校正過的欄位會列出來：

```
EKBE：920,000 列 <- 1 個檔案
  型別調整：MENGE->DECIMAL(18,3)（掃描只看前 200 列，後面才出現的值放不進原本判的型別）
```

文字超過 4000 字才給 `NVARCHAR(MAX)` —— MAX 欄位不能當索引鍵，不隨便用。

你在 `config.json` 指定死的型別**不會被偷偷改掉**，放不下時會報錯並指名欄位：

```
指定的型別放不下：MENGE 指定 INT，但實際資料需要 DECIMAL(18,1)。
